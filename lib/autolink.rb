...
class AutolinkFilter < Nanoc::Filter
	...
	AUTOLINK_SKIP = ["a", "pre", "code", "script"].freeze
	...
	AUTOLINK_PATTERNS = {
		/(?<!\w)#[\.\-\/:_a-zA-Z0-9]+?[a-zA-Z][\.\-\/:_a-zA-Z0-9]+/ => :autolink_hashtag,
		/(?<!\w)@[\.\-\/:__a-zA-Z0-9]+/ => :autolink_atreply,
		URL_PATTERN => :autolink_url
	}.freeze

	def run(content, _params)
		AUTOLINK_PATTERNS.reduce(Nokogiri::HTML::DocumentFragment.parse(content)) do |doc, (pattern, callback)|
			doc.traverse do |node|
				if node.name == "text" && !AUTOLINK_SKIP.any? { |tag| node.ancestors.any? { |up| up.name == tag } }
					replacement = node.to_xhtml.gsub(pattern) { |txt| send(callback, txt) }
